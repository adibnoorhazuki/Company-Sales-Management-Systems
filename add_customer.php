        ...
        <div class="col-md-10" style="margin: 0 auto;">
          <div class="card " >
            <div class="card-header">
              ...
              if($type == "update"){
                ?>
                <h3 class="card-title"><b>Update Customer</b></h3>
                <?php
              }
              else{
                ?>
                <h3 class="card-title"><b>Add Customer</b></h3>
                <?php
              }
              ?>
            ...
            <div class="card-body">
                ...
                <div class="col-lg-12">
                  ...
                  if($type == "update"){
                    ?>
                    <form id="add_dept" method="post" action="update_process.php?type=cust">
                    <?php
                  }
                  else{
                    ?>
                    <form id="add_dept" method="post" action="add_process.php?type=cust">
                    <?php
                  }
                  
                  if($type == "update"){
                    $update = "SELECT * FROM customer WHERE cust_id = '".$id."'";
                    $rsltupdate = $conn->query($update);
                    if($rsltupdate->num_rows>0){
                      while($rowupdate = $rsltupdate->fetch_assoc()){
                        $name = $rowupdate["cust_name"];
                        $add = $rowupdate["cust_address"];
                        $phone = $rowupdate["cust_phone"];
                        $email = $rowupdate["cust_email"];
                      }
                    }
                  }
                  ?>
                    <div class="form-group">
                      ...
                      <input type="text" id="cust_name" name="cust_name" class="form-control" value="<?php echo $name ?>" required>
                    </div>
  
                    <div class="form-group">
                      <label for="cust_add">Customer Address</label>
                      <input type="text" id="cust_add" name="cust_add" class="form-control" value="<?php echo $add ?>" required>
                    </div>

                    <div class="form-group">
                      <label for="cust_phone">Customer Phone Number</label>
                      <input type="text" id="cust_phone" name="cust_phone" class="form-control" value="<?php echo $phone ?>" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"  required>
                    </div>

                    <div class="form-group">
                      <label for="cust_email">Customer Email</label>
                      <input type="email" id="cust_email" name="cust_email" class="form-control" value="<?php echo $email ?>" required>
                    </div>
                    <?php
                    if($type == "update"){
                      ?>
