        ...
        <div class="col-md-10" style="margin: 0 auto;">
          <div class="card " >
            <div class="card-header">
              ...
              if($type == "update"){
                ?>
                <h3 class="card-title"><b>Update Department</b></h3>
                <?php
              }
              else{
                ?>
                <h3 class="card-title"><b>Add Department</b></h3>
                <?php
              }
              ?>
            ...
            <div class="card-body">
                ...
                <div class="col-lg-12">
                  ...
                  if($type == "update"){
                    ?>
                    <form id="add_dept" method="post" action="update_process.php?type=dept&deptid=<?php echo $id ?>">
                    <?php
                  }
                  else{
                    ?>
                    <form id="add_dept" method="post" action="add_process.php?type=dept">
                    <?php
                  }
                  ?>
                    <?php
                    if($type == "update"){
                      $update = "SELECT * FROM department WHERE dept_id = '".$id."'";
                      $rsltupdate = $conn->query($update);
                      if($rsltupdate->num_rows>0){
                        while($rowupdate = $rsltupdate->fetch_assoc()){
                          $name = $rowupdate["dept_name"];
                          $desc = $rowupdate["dept_desc"];
                        }
                      }
                    }
                    ?>
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" id="dept_name" name="dept_name" class="form-control" value="<?php echo $name ?>" required>
                    </div>
  
                    <div class="form-group">
                      <label for="dept_desc">Department Description</label>
                      <input type="text" id="dept_desc" name="dept_desc" class="form-control" value="<?php echo $desc ?>" required>
                    </div>
                    <?php
                    if($type == "update"){
                      ?>
